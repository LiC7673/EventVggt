...
def train(args):
    ...
    def write_log_stats(epoch, train_stats, test_stats):
        if accelerator.is_main_process:
            ...
            # Save test metrics to separate files
            for test_name, test_metrics in test_stats.items():
                test_log_file = os.path.join(args.output_dir, f"test_metrics_{test_name}.json")
                test_metrics_with_epoch = {"epoch": epoch, **test_metrics}
                    ...
                    with open(test_log_file, "r", encoding="utf-8") as f:
                        try:
                            ...
                            if isinstance(existing_metrics, list):
                                existing_metrics.append(test_metrics_with_epoch)
                            else:
                                existing_metrics = [existing_metrics, test_metrics_with_epoch]
                        except:
                            existing_metrics = [test_metrics_with_epoch]
                else:
                    existing_metrics = [test_metrics_with_epoch]
            ...
            # Save train metrics separately
            if train_stats:
                train_log_file = os.path.join(args.output_dir, "train_metrics.json")
                train_metrics_with_epoch = {"epoch": epoch, **train_stats}
                ...
                if os.path.exists(train_log_file):
                    with open(train_log_file, "r", encoding="utf-8") as f:
                        try:
                            existing_metrics = json.load(f)
                            if isinstance(existing_metrics, list):
                                existing_metrics.append(train_metrics_with_epoch)
                            else:
                                existing_metrics = [existing_metrics, train_metrics_with_epoch]
                        except:
                            existing_metrics = [train_metrics_with_epoch]
                else:
                    existing_metrics = [train_metrics_with_epoch]
            ...
            printer.info(f"Test metrics saved to output_dir")
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        if epoch > 0 and args.eval_freq > 0 and epoch % args.eval_freq == 0:
            ...
            save_test_summary(args, epoch, test_stats)
...
def save_test_summary(args, epoch, test_stats):
    """Save test summary report and visualization metrics."""
    if not test_stats:
        return
    ...
    with open(summary_file, "a", encoding="utf-8") as f:
        f.write(f"\n{'='*80}\n")
        f.write(f"Epoch {epoch} - Test Results Summary\n")
        f.write(f"{'='*80}\n")
        
        for test_name, metrics in test_stats.items():
            f.write(f"\n{test_name}:\n")
            f.write(f"{'-'*40}\n")
