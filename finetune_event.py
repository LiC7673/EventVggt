...
from eventvggt.models.streamvggt_reliability_filter_detail import StreamVGGT as EventReliabilityFilterDetailStreamVGGT
...
def build_event_model(cfg) -> nn.Module:
    ...
    if variant in ("reliability_filter_detail", "temporal_reliability_filter_detail", "filtered_temporal_detail"):
        return EventReliabilityFilterDetailStreamVGGT(
            **common_kwargs,
            event_num_bins=int(getattr(cfg.model, "event_num_bins", default_event_bins)),
            event_count_cmax=float(getattr(cfg.model, "event_count_cmax", 3.0)),
            residual_scale=float(getattr(cfg.model, "refiner_residual_scale", 0.03)),
            gate_downsample=int(getattr(cfg.model, "event_gate_downsample", 2)),
            event_reliability_floor=float(getattr(cfg.model, "event_reliability_floor", 0.30)),
            event_reliability_init_bias=float(getattr(cfg.model, "event_reliability_init_bias", 0.5)),
            refine_points=bool(getattr(cfg.model, "refiner_refine_points", True)),
            use_checkpoint=bool(getattr(cfg.model, "refiner_use_checkpoint", True)),
        )
    ...
    if variant in ("temporal_reliability_v2", "specular_reliability_v2", "multi_ldr_reliability_v2"):
        return EventTemporalReliabilityV2StreamVGGT(
            **common_kwargs,
            ...
            proposal_use_depth_hf=bool(getattr(cfg.model, "proposal_use_depth_hf", False)),
            event_proposal_weight=float(getattr(cfg.model, "event_proposal_weight", 0.0)),
            event_delta_highpass_kernel=int(getattr(cfg.model, "event_delta_highpass_kernel", 0)),
            event_delta_patch_zero_mean=bool(getattr(cfg.model, "event_delta_patch_zero_mean", False)),
            event_delta_patch_size=int(getattr(cfg.model, "event_delta_patch_size", getattr(cfg.model, "patch_size", 14))),
            event_gate_smooth_kernel=int(getattr(cfg.model, "event_gate_smooth_kernel", 5)),
            final_degrid_strength=float(getattr(cfg.model, "final_degrid_strength", 0.0)),
            final_degrid_kernel=int(getattr(cfg.model, "final_degrid_kernel", 5)),
            forward_batch_chunk=int(getattr(cfg.model, "exposure_forward_batch_chunk", 1)),
            refine_points=bool(getattr(cfg.model, "refiner_refine_points", True)),
            use_checkpoint=bool(getattr(cfg.model, "refiner_use_checkpoint", True)),
        )
