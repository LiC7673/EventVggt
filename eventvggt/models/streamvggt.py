...
class StreamVGGT(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(
        ...
        if event_features is not None:
            ...
            event_tokens = event_tokens.flatten(2).transpose(1, 2)
            ...
            event_tokens_padded = torch.cat([cls_pad, event_tokens], dim=1) # -> [B*S, N+1, embed_dim]
            ...
            for s in range(S):
                res = {
                    ...
                    **({'pts3d_in_other_view': predictions['world_points'][:, s],
                    ...
                    if 'world_points' in predictions else {}),
    ...
    def inference(self, frames, query_points: torch.Tensor = None, past_key_values=None):        
        ...
        for i, frame in enumerate(frames):
            ...
            if self.point_head is not None:
                res.update({'pts3d_in_other_view': pts3d, 'conf': pts3d_conf})
            all_ress.append(res)
