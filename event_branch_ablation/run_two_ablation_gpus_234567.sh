...
env CUDA_VISIBLE_DEVICES="${GEOMETRY_GPUS}" HYDRA_FULL_ERROR=1 PYTHONUNBUFFERED=1 \
  ...
  exp_name=geometry_motion_full_img_reliability_v5_stable_scene12 epochs="${EPOCHS}" \
  hydra.run.dir="${ROOT_DIR}/abl_event_exp/geometry_motion_full_img_reliability_v5_stable_scene12" \
...
env CUDA_VISIBLE_DEVICES="${DECOMPOSE_GPUS}" HYDRA_FULL_ERROR=1 PYTHONUNBUFFERED=1 \
  ...
  exp_name=full_to_additive_tokens_img_reliability_v4_stable_scene12 epochs="${EPOCHS}" \
  hydra.run.dir="${ROOT_DIR}/abl_event_exp/full_to_additive_tokens_img_reliability_v4_stable_scene12" \
  data.root="${DATA_ROOT}" data.num_views="${NUM_VIEWS}" data.ldr_event_id="${LDR_ID}" \
  data.initial_scene_idx=0 data.active_scene_count="${ACTIVE_SCENE_COUNT}" \
  num_workers="${NUM_WORKERS}" pin_mem=false \
