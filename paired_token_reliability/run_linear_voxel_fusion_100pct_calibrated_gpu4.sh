#!/usr/bin/env bash
# Additive calibrated version of fusion-100%; the old *_fixed run is untouched.
set -Eeuo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"; cd "${ROOT}"
export GPUS=4
export OUTPUT="${OUTPUT:-exp/linear_voxel_fusion_100pct_gpu4_calibrated}"
export TRAIN_MODULE="paired_token_reliability.train_linear_voxel_calibrated"
export RUN_EVAL=0
...
CUDA_VISIBLE_DEVICES=4 python -m paired_token_reliability.evaluate_linear_voxel_calibrated \
