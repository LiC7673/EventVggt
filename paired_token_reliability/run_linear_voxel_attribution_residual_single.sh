...
export GPUS="${GPUS:-1}"
EXPERIMENT="${EXPERIMENT:-attribution_residual_full_gpu1}"
export OUTPUT="${OUTPUT:-exp/${EXPERIMENT}}"
export TRAIN_MODULE="paired_token_reliability.train_linear_voxel_attribution_residual"
export RUN_EVAL=0
export EPOCHS_A="${EPOCHS_A:-12}" EPOCHS_B=0 EPOCHS_C=0
ABLATE_ATTRIBUTION="${ABLATE_ATTRIBUTION:-false}"
ABLATE_RESIDUAL="${ABLATE_RESIDUAL:-false}"
...
echo "[run] gpu=${GPUS} output=${OUTPUT} pretrained=${PRETRAINED} ablate_C=${ABLATE_ATTRIBUTION} ablate_residual=${ABLATE_RESIDUAL}"
bash paired_token_reliability/run_linear_voxel_multiscale_12train_4test.sh \
  ...
  "model.geometry_projection_dim=256" \
  ...
  "model.ablate_event_attribution=${ABLATE_ATTRIBUTION}" \
  "model.ablate_missing_residual=${ABLATE_RESIDUAL}" \
  "${RESUME_ARGS[@]}" "$@"

CUDA_VISIBLE_DEVICES="${GPUS}" python -m paired_token_reliability.evaluate_linear_voxel_attribution_residual \
