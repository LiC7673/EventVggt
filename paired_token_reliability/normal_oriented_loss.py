...
class NormalOrientedGeometryLoss(UnifiedGeometryContributionLoss):
    def __init__(self, *, normal_gradient_weight=0.5, depth_normal_consistency_weight=0.5,
                 normal_update_weight=0.01, outside_support_weight=0.2,
        ...
        self.normal_update_weight = float(normal_update_weight)
        self.outside_support_weight = float(outside_support_weight)
    ...
    def geometry(self, output, views, bridge_mask):
        ...
        magnitude = _weighted_mean(delta.float().abs().mean(-1), valid)
        outside = _weighted_mean((1.0 - support) * delta.float().abs().mean(-1), valid)
        ...
        total = (base + self.normal_gradient_weight * normal_grad
                 ...
                 + self.normal_update_weight * magnitude
                 + self.outside_support_weight * outside
        ...
        details.update(normal_gradient=normal_grad, depth_normal_consistency=consistency,
                       normal_update_magnitude=magnitude, outside_support=outside,
