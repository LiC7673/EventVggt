#!/usr/bin/env bash
# Protect event geometry from hard mean projection. No visualization changes.
set -Eeuo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"; cd "${ROOT}"
export GPUS=4
export OUTPUT="${OUTPUT:-exp/linear_voxel_conditioned_soft_dc_gpu4}"
export TRAIN_MODULE="paired_token_reliability.train_linear_voxel_conditioned_soft_dc"
export RUN_EVAL=0
bash paired_token_reliability/run_linear_voxel_multiscale_12train_4test.sh \
  ...
  "model.event_dc_limit=0.03" "$@"

CUDA_VISIBLE_DEVICES=4 python -m paired_token_reliability.evaluate_linear_voxel_conditioned_soft_dc \
