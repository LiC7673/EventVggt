#!/usr/bin/env bash
# V10 comparison: event branch predicts dN/dx,dN/dy, never absolute normals.
set -Eeuo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"; cd "${ROOT}"
export GPUS="${GPUS:-4}"
export OUTPUT="${OUTPUT:-exp/linear_voxel_dual_alignment_hdr_direct_normal_derivative_gpu4}"
export TRAIN_MODULE="paired_token_reliability.train_linear_voxel_dual_alignment_hdr_derivative"
