#!/usr/bin/env bash
# Coarse-conditioned event refinement, scheduled event-detail supervision, and
# update-stage diagnostics.  Visualization behavior is intentionally unchanged.
set -Eeuo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"; cd "${ROOT}"
export GPUS=1
export OUTPUT="${OUTPUT:-exp/linear_voxel_conditioned_scheduled_gpu4}"
export TRAIN_MODULE="paired_token_reliability.train_linear_voxel_conditioned_scheduled"
export RUN_EVAL=0
...
CUDA_VISIBLE_DEVICES=4 python -m paired_token_reliability.evaluate_linear_voxel_conditioned_scheduled \
