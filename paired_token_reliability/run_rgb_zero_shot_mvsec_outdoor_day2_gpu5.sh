#!/usr/bin/env bash
# Pure-RGB zero-shot evaluation on MVSEC outdoor_day2.
# No event input and no model parameter update.
set -Eeuo pipefail
...
OUTPUT="${OUTPUT:-exp_f/hdr_diff_rgb_zero_shot_mvsec_outdoor_day2_scale20_gpu5}"
RGB_DIRECTORY="${RGB_DIRECTORY:-${MVSEC_RAW_ROOT}/hdr_diff_images/outdoor_day2/event_aligned}"

if [[ ! -f "${PRETRAINED}" ]]; then
  echo "Missing pretrained RGB checkpoint: ${PRETRAINED}" >&2
  exit 2
fi
if [[ ! -d "${RGB_DIRECTORY}" ]]; then
  echo "Missing HDR-Diff MVSEC RGB directory: ${RGB_DIRECTORY}" >&2
  exit 2
fi

# Only the converted RGB/depth/pose streams are used by the evaluator.
if [[ ! -s "${MVSEC_H5_ROOT}/outdoor_day2_data.hdf5" || \
      ! -s "${MVSEC_H5_ROOT}/outdoor_day2_gt.hdf5" ]]; then
  echo "[MVSEC RGB zero-shot] converting outdoor_day2 with rosbags"
...
echo "[MVSEC RGB zero-shot] sequence=outdoor_day2"
echo "[MVSEC RGB zero-shot] external RGB=${RGB_DIRECTORY}"
echo "[MVSEC RGB zero-shot] epochs=0; parameter updates=NONE; event input=NONE"
echo "[MVSEC RGB zero-shot] scale calibration frames=${SCALE_CALIBRATION_FRAMES:-20}"
...
python -m paired_token_reliability.finetune_rgb_real_dataset \
  ...
  --mvsec-rgb-directory "${RGB_DIRECTORY}" \
  --epochs 0 \
  --max-train-steps 0 \
