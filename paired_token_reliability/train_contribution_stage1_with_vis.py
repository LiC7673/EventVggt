...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--vis-every-epochs", type=int, default=5)
...
def _event_projection(event):
    if event.dim() == 4:
        event = event[0]
    return event.abs().sum(dim=0).detach().float().cpu().numpy()


def _contribution_projection(contribution, event):
    if contribution.dim() == 4:
        contribution = contribution[0]
    if event.dim() == 4:
        event = event[0]
    c = contribution.detach().float()
    e = event.detach().float().abs()
    return ((c * e).sum(dim=0) / (e.sum(dim=0) + 1e-6)).cpu().numpy()


@torch.no_grad()
def visualize_contribution_samples(model, rgb_model, loader, device, args, phase, epoch):
    out = Path(args.vis_output) if args.vis_output else Path(args.output) / "vis"
    save_dir = out / phase / f"epoch_{epoch+1:03d}"
    ...
    for batch in loader:
        if saved >= args.vis_num_samples:
            break

        prepared, _, prediction, _ = process_batch(
            model, rgb_model, batch, device, args, phase
        )

        c_map = _contribution_projection(
            prediction["contribution"],
            prepared["event"]
        )
        event_map = _event_projection(prepared["event"])
        selected_map = _event_projection(
            prepared["event"] * prediction["contribution"]
        )

        bridge = prepared["bridge"].bridge[0].float().cpu().numpy()
        rgb_bad = prepared["rgb_bad"][0, 0].permute(1,2,0).cpu().numpy()
        rgb_ref = prepared["rgb_reference"][0,0].permute(1,2,0).cpu().numpy()

        fig, ax = plt.subplots(2,3, figsize=(12,7))

        imgs = [
            (rgb_bad, "RGB bad"),
            (rgb_ref, "RGB reference"),
            (bridge, "Bridge mask"),
            (event_map, "Event projection"),
            (c_map, "Contribution map"),
            (selected_map, "Selected event"),
        ]

        for a, (img, title) in zip(ax.flat, imgs):
            a.imshow(img)
            a.set_title(title)
            a.axis("off")

        fig.tight_layout()
        fig.savefig(save_dir / f"sample_{saved:02d}.png", dpi=200)
        plt.close(fig)
