"""Held-out evaluator for the explicitly supervised detail-residual model."""
...
def build_model(checkpoint: Path, _override, device):
    ...
    expected = DetailResidualLinearVoxelModel.checkpoint_schema
    ...
    model = DetailResidualLinearVoxelModel(
        ...
        depth_update_scale=float(getattr(m, "depth_update_scale", 1.0)),
    ...
    print(f"[detail eval] learned depth scale={float(model.metric_depth_scale):.6f}", flush=True)
