#!/usr/bin/env bash
# Event feature -> event normal -> depth. No direct event-to-depth shortcut.
set -Eeuo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"; cd "${ROOT}"

export GPUS="${GPUS:-4}"
export OUTPUT="${OUTPUT:-exp/linear_voxel_normal_bottleneck_refine_gpu4}"
export TRAIN_MODULE="paired_token_reliability.train_linear_voxel_normal_bottleneck_refine"
export RUN_EVAL=0
...
if [[ "${RESUME}" == "auto" ]]; then
  ...
  if [[ -n "${LATEST}" ]]; then
    ...
    echo "[resume] no normal-bottleneck checkpoint found; starting fresh"
  fi
...
CUDA_VISIBLE_DEVICES="${GPUS}" python -m paired_token_reliability.evaluate_linear_voxel_normal_bottleneck_refine \
