#!/usr/bin/env bash
# Same-scale RGB coarse + directly supervised event detail residual on GPU 4.
set -Eeuo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"; cd "${ROOT}"
export GPUS=4
export OUTPUT="${OUTPUT:-exp/linear_voxel_detail_residual_gpu4}"
export TRAIN_MODULE="paired_token_reliability.train_linear_voxel_detail_residual"
export RUN_EVAL=0
...
CUDA_VISIBLE_DEVICES=4 python -m paired_token_reliability.evaluate_linear_voxel_detail_residual \
