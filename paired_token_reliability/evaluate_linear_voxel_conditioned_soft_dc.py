"""Evaluator for conditioned soft-DC event geometry checkpoints."""
...
def build_model(checkpoint: Path, _override, device):
    ...
    expected = ConditionedSoftDCLinearVoxelModel.checkpoint_schema
    ...
    model = ConditionedSoftDCLinearVoxelModel(
        ...
        event_dc_limit=float(getattr(m, "event_dc_limit", .03)),
        event_residual_target_limit=float(
            getattr(m, "event_residual_target_limit", .25)
        ),
