...
CHECKPOINT="${CHECKPOINT:?Set CHECKPOINT to the unified checkpoint path}"
OUTPUT_DIR="${OUTPUT_DIR:?Set OUTPUT_DIR for evaluation results}"
GPU="${GPU:-0}"

export PYTHONPATH="${ROOT}:${PYTHONPATH:-}"
CUDA_VISIBLE_DEVICES="${GPU}" python -m paired_token_reliability.evaluate_unified_all_exposures \
  ...
  --visualize-every "${TEST_VIS_EVERY:-1}" \
  "$@"
