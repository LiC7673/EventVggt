...
SCRIPTS=(
  "mul_loss_fine/finetune_mul_loss_baseline.py"
  "mul_loss_fine/finetune_mul_loss_mv_normal.py"
  "mul_loss_fine/finetune_mul_loss_mv_presence.py"
  "mul_loss_fine/finetune_mul_loss_mv_hf.py"
  "mul_loss_fine/finetune_mul_loss_mv_presence_hf.py"
  ...
  "mul_loss_fine/finetune_mul_loss_detail_gt_temporal_detail.py"
...
if [[ "${#GPUS[@]}" -eq 0 ]]; then
  echo "No GPUs configured. Set GPU_LIST, for example: GPU_LIST=0,1,2,3 bash mul_loss_fine/run_mul_loss_2gpu_8gpu.sh"
  exit 1
fi
...
LOG_DIR="${ROOT_DIR}/ablation_logs/mul_loss_2gpu_${RUN_ID}"
mkdir -p "$LOG_DIR"
...
if [[ "$STATUS" -eq 0 ]]; then
  echo "All mul-loss jobs finished. Logs are in ${LOG_DIR}"
else
  echo "Some mul-loss jobs failed. Check logs in ${LOG_DIR}"
fi
