...
`mul_loss_fine` 的核心思想是：**事件流不直接监督深度，而是作为细节区域的可靠性权重**。如果指标提升但法向细节仍然不清晰，优先看 `detail_gt` 和 `mv_all_detail_gt`；如果网格伪影在纯 RGB 中也出现，优先看 `antigrid` 模型变体或 patch-grid 抑制，而不是继续加事件 loss。
