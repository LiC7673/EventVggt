"""Temporal detail with image-guided event reliability.

This keeps the simple temporal-detail residual path, but adds a lightweight
learned reliability gate supervised by GT geometry detail and image cues.  It
uses longer training by default because the reliability head needs time to
separate useful geometric events from highlight/noise events.
"""
...
def run(cfg: OmegaConf):
    ...
    cfg = configure_mul_loss_cfg(
        _prepare_cfg(cfg),
        weights=weights,
        exp_name="mul_loss_detail_gt_temporal_detail_img_reliability",
    )
    ...
    print(
        "Temporal-detail image-guided reliability training: no v2 counterfactual; "
        "events write bounded high-pass residuals gated by image/geometry-supervised reliability; "
        f"epochs={cfg.epochs}, pretrained={cfg.pretrained}"
    )
    fe.train(cfg)
